...
pub const Compiler = struct {
    ...
    fn compileFromNode(self: *Compiler, node: *ir_mod.Node, dst: u16, is_tail: bool) CompileError!void {
        switch (node.tag) {
            ...
            .letrec_star => try forms.compileLetrecStar(self, node.data.letrec_star.args, dst, is_tail),
            .do_form => try forms.compileDo(self, node.data.do_form.args, dst, is_tail),
            ...
            .cond => try forms.compileCond(self, node.data.cond.args, dst, is_tail),
            .case_form => try forms.compileCase(self, node.data.case_form.args, dst, is_tail),
            .case_lambda => try forms.compileCaseLambda(self, node.data.case_lambda.args, dst),
            .guard => try forms.compileGuard(self, node.data.guard.args, dst, is_tail),
            .quasiquote => try advanced.compileQuasiquote(self, node.data.quasiquote.args, dst),
            .parameterize => try advanced.compileParameterize(self, node.data.parameterize.args, dst, is_tail),
            .define_values => try self.compileDefineValues(node.data.define_values.args, dst),
            .let_values => try forms.compileLetValues(self, node.data.let_values.args, dst, is_tail),
            .let_star_values => try forms.compileLetStarValues(self, node.data.let_star_values.args, dst, is_tail),
            .define_syntax => try self.compileDefineSyntax(node.data.define_syntax.args, dst),
            .named_let => try forms.compileNamedLet(self, node.data.named_let.args, dst, is_tail),
            .let_syntax => try self.compileLetSyntax(node.data.let_syntax.args, dst, is_tail),
            .letrec_syntax => try self.compileLetrecSyntax(node.data.letrec_syntax.args, dst, is_tail),
