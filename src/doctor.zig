...
fn collectNativeBackend(r: *Report) void {
    ...
    // #1656: on an arch the LLVM backend can't target (s390x, ppc64le),
    ...
    if (!llvm_emit.native_backend_supported) {
        r.add(
            ...
            "arch",
            .warn,
            r.fmt("native compilation unavailable on {s} — the LLVM backend targets aarch64, x86_64 and riscv64 (Linux) only; the interpreter tier is fully supported", .{@tagName(builtin.cpu.arch)}),
