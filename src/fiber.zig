...
// The blocking-wait machinery — fd waits (waitForFd, wakeIoWaitersOnFd),
// reactor parking (parkOnReactor), and the shared in-place scheduler drive
// (runSchedulerStep) — lives in fiber_wait.zig. Same-name aliases keep every
// external fiber.X reference resolving.
const fiber_wait = @import("fiber_wait.zig");
pub const wakeIoWaitersOnFd = fiber_wait.wakeIoWaitersOnFd;
pub const IoWait = fiber_wait.IoWait;
pub const waitForFd = fiber_wait.waitForFd;
...
/// can pick it up. Shared by parkOnReactor (blocking wait, fiber_wait.zig)
/// and schedule()'s per-tick expired-timer check (non-blocking) — pub for
/// the former.
pub fn wakeReadyFiber(f: *Fiber) void {
    switch (f.status) {
        .io_waiting => {
            f.status = .suspended;
            f.io_fd = null;
