...
// -- Import-set algebra (delegated to vm_imports.zig) --
//
// Aliased under their original names so both external `vm_library.X` and
// this file's own internal references keep resolving unchanged.
const vm_imports = @import("vm_imports.zig");
pub const ImportTracker = vm_imports.ImportTracker;
pub const importSetChecked = vm_imports.importSetChecked;
pub const handleImportInto = vm_imports.handleImportInto;
pub const handleImport = vm_imports.handleImport;
pub const processImportSet = vm_imports.processImportSet;
...
pub fn importSrfi261Normalized(vm: *VM, target: *std.StringHashMap(Value), import_set: Value, lib_name: []const u8, n: i64) VMError!void {
