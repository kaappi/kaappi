...
- CI quality gates: formatting check, Debug/ReleaseFast optimize matrix, build
  caching
- `--experimental-threads` flag to gate OS threads (SRFI-18) until cross-thread
  GC is safe
...
Initial release. Complete R7RS-small implementation with 554 built-in
procedures, 32 syntax forms, 14 standard libraries, 51 SRFIs, C FFI,
JIT compiler, green threads, profiler, stepping debugger, bytecode caching,
and standalone binary bundling.
