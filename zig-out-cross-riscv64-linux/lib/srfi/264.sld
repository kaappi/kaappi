;;; SRFI 264: String Syntax for Scheme Regular Expressions (SSRE)
;;;
;;; A compact, PCRE-inspired string syntax for regular expressions that
;;; translates to the S-expression SRE notation of SRFI 115.
;;;
;;; This is a faithful port of the SRFI 264 reference implementation by
;;; Sergei Egorov, wrapped in an R7RS define-library. The body below (helpers,
;;; parser, unparser, and the public entry points) is kept close to upstream
;;; so it can be re-synced with future revisions of the reference source.
;;;
;;; SPDX-FileCopyrightText: 2025 Sergei Egorov
;;; SPDX-License-Identifier: MIT
(define-library (srfi 264)
  (import (scheme base) (scheme char) (scheme write) (scheme cxr) (srfi 115))
  (export ssre->sre ssre->regexp sre->ssre
          ssre-definitions ssre-bind ssre-unbind)
  (begin
; Helpers
...
(define (ssre-syntax-error? x)
  ;; Kaappi fix vs upstream: `fail` raises the remaining source as a char
  ;; list, and `ssre-fancy-error` consumes it with `length`, so the src field
  ;; (cadr x) is a list, never a string. Upstream's `(string? (cadr x))` can
  ;; never match, leaving this guard dead so a raw list escapes instead of a
  ;; formatted error object. Check `list?` so ssre-fancy-error actually runs.
  (and (list? x) (= (length x) 4) (eq? (car x) 'ssre->sre)
       (list? (cadr x)) (string? (caddr x)) (list? (cadddr x))))
