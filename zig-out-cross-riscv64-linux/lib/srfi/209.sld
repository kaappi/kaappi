...
(define-library (srfi 209)
  ...
  (begin
    ...
    ;;;
    ;;; `ctor-name` resolves each name via `(type-name nm)` rather than
    ;;; quoting `nm` itself. This started as a workaround for an expander
    ;;; bug (#1716: a self-recursive macro generated by an enclosing macro
    ;;; re-quoted the literal pattern-variable name "nm" instead of the
    ;;; captured identifier on the recursive call) — that bug is fixed
    ;;; (#1773), and `tests/scheme/hygiene/recursive-generated-macro-quote-1716.scm`
    ;;; keeps it fixed, so `'nm` would work here now. The routing is kept
    ;;; deliberately: `type-name`'s clauses list the enum names as
    ;;; syntax-rules literals, so a name that is not in the enum type is
    ;;; rejected at expansion time, where quoting `nm` directly would only
    ;;; surface it later as a runtime failure inside `enum-set-adjoin`.
