...
;;;
;;; Ported from the sample implementation by Wolfgang Corcoran-Mathe
;;; (https://github.com/scheme-requests-for-implementation/srfi-189),
;;; following the specification exactly.
;;;
;;; A Just, Right, or Left container may hold zero or more payload
;;; objects; the payload is stored internally as a list.  A Just with
;;; no payload is not the same as Nothing: it represents success with
;;; no values.

(define-library (srfi 189)
  (import (scheme base) (scheme case-lambda) (srfi 1) (srfi 145))
  (export
    just nothing left right
    just? nothing? left? right? maybe? either?
    maybe= either= either-swap
    maybe-ref maybe-ref/default either-ref either-ref/default
    maybe-join maybe-bind maybe-compose either-join either-bind either-compose
    maybe-length maybe-filter maybe-remove
    either-length either-filter either-remove
    maybe-sequence either-sequence
    maybe->either either->maybe list->just list->left list->right
    ...
    maybe-map maybe-for-each maybe-fold maybe-unfold
    either-map either-for-each either-fold either-unfold
    maybe-if maybe-and maybe-or maybe-let* maybe-let*-values
  ...
  (begin
    ...
    ;; Per SRFI-189, nothing is a procedure: (nothing) returns the
    ;; unique Nothing object.
    ...
    ;;; Caller-provided list arguments are copied so that later
    ;;; mutation of the list argument doesn't affect the payload.
    ...
    ;; True if all the maybes are Nothing, or if all are Justs
    ;; containing the same number of values which are element-wise
    ...
    ;; True if the eithers are all Lefts/all Rights containing the
    ;; same number of values which are element-wise equal in the
    ;; sense of `equal'.
    ...
    ;; Unwrap an Either, calling failure on the payload of a Left and
    ;; the optional success continuation (default: values) on that of
    ;; a Right.
    ...
    ;; If maybe is a Just containing a single Maybe, return that
    ;; Maybe.
    ...
    ;; Call the first mproc on the payload of a Maybe, producing a
    ;; Maybe.  Repeat the operation with this Maybe and the remaining
    ;; mprocs.  Return a Nothing immediately.
    ...
    ;; Compose the argument mprocs and return the resulting monadic
    ;; procedure.
    ...
    ;; If either is a Right containing a single Either, return that
    ;; Either.
    ...
    ;; Call the first mproc on the payload of an Either, producing an
    ;; Either.  Repeat the operation with this Either and the
    ;; remaining mprocs.  Return a Left immediately.
    ...
    ;; Return maybe if its payload satisfies pred; otherwise, return
    ;; Nothing.
    ...
    ;; Return maybe if its payload doesn't satisfy pred; otherwise,
    ;; return Nothing.
    ...
    ;; Return either if its payload doesn't satisfy pred; otherwise,
    ;; return a Left of the default objects.
    ...
    ;; objects with aggregator, and wrap the new collection in a
    ;; Right.  If a Left is encountered while traversing, return it
    ;; immediately.
    ...
    (define (maybe->list maybe)
      (assume (maybe? maybe))
      (if (nothing? maybe) '() (list-copy (just-objs maybe))))

    (define (either->list either)
      (assume (either? either))
      (list-copy ((if (right? either) right-objs left-objs) either)))
    ...
    ;; If maybe is a Just, return its payload; otherwise, return
    ;; false.
    ...
    ;; If either is a Right, return its payload; otherwise, return
    ;; false.
    ...
    ;;; The list-truth protocol uses #f to represent failure and a
    ;;; list to represent success.

    (define (maybe->list-truth maybe)
      (assume (maybe? maybe))
      (if (just? maybe) (list-copy (just-objs maybe)) #f))

    (define (either->list-truth either)
      (assume (either? either))
      (if (right? either) (list-copy (right-objs either)) #f))

    ;; If list-or-false is #f, return Nothing.  If it's a list, return
    ;; a Just containing its elements.
    ...
    ;;; The generation protocol uses an EOF object to represent
    ...
    ;; If either is a Right whose payload is a single value, return
    ;; that value.  If it's a Left, return an eof-object.
    ...
    ;;; The values protocol uses one or more values to represent
    ...
    ;;; The two-values protocol returns |#f, #f| to represent failure
    ;;; and |<any object>, #t| to represent success.

    ;; If maybe is a Just containing a single value, return that value
    ;; and #t.  If it's a Nothing, return |#f, #f|.
    ...
    ;; If maybe is a Just, apply proc to its payload and wrap the
    ;; result in a Just.  Otherwise, return Nothing.
    ...
    ;; successor on seeds and apply stop? to the results; if stop?
    ;; returns true, apply mapper to seeds and return the results
    ;; wrapped in a Just.
    ...
    ;; If either is a Right, apply proc to its payload and wrap the
    ;; result in a Right.  Otherwise, return either.
    ...
    ;; If the seeds satisfy stop?, return a Left of seeds.  Otherwise,
    ;; call successor on seeds and apply stop? to the results; if
    ;; stop? returns true, apply mapper to seeds and return the
    ;; results wrapped in a Right.
    ...
    ;; Maybe analog of and.  Evaluate the argument expressions in
    ;; order.  If any expression evaluates to Nothing, return it.
    ;; Otherwise, return the last Just.
    ...
    ;; Maybe analog of or.  Evaluate the argument expressions in
    ;; order.  If any expression evaluates to a Just, return it
    ;; immediately.  Otherwise, return Nothing.
    ...
    ;; Maybe analog of SRFI 2's and-let*.  Each claw evaluates an
    ;; expression or bound variable to a Maybe, or binds the payload
    ;; of the value of a Maybe expression to a name in later claws and
    ;; the body.  If any claw gives a Nothing, the whole expression
    ...
    ;; binds the payload of the value of <maybe-expr> to <formals> in
    ;; the manner of let-values.
    ...
    ;; Either analog of and.  Evaluate the argument expressions in
    ;; order.  If any expression evaluates to a Left, return it
    ;; immediately.  Otherwise, return the last Right.
    ...
    ;; Either analog of or.  Evaluate the argument expressions in
    ;; order.  If any expression evaluates to a Right, return it
    ;; immediately.  Otherwise, return the last Left.
    ...
    ;; Either analog of SRFI 2's and-let*.  Each claw evaluates an
    ;; expression or bound variable to an Either, or binds the payload
    ;; of the value of an Either expression to a name in later claws
    ;; and the body.  If any claw gives a Left, then the whole
    ;; expression evaluates to that Left.
    ...
    ;; Like either-let*, but a claw of the form (<formals>
    ;; <either-expr>) binds the payload of the value of <either-expr>
    ;; to <formals> in the manner of let-values.
    ...
    ;;; In the following procedures, (just #f) is considered to be
    ;;; false.  All other Just values are taken to be true.
    ...
    ;; Returns #t if all arguments are true or all false.  If any
    ;; argument is Nothing or if any two arguments have different
    ;; (tri-)truth values, #f is returned.
    (define (tri=? maybe . ms)
      (let ((make-pred
             (lambda (b)
               (lambda (m)
    ...
    ;; Returns #t if all arguments are true.  If any argument is false
    ;; or Nothing, return the first such object.
    ...
    ;; Returns #f if all arguments are false.  If any argument is true
    ;; or Nothing, return the first such object.
