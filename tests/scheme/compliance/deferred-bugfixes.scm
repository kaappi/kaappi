...
(let ((p (open-output-bytevector)))
  ...
  (test #u8(20 30 40) (get-output-bytevector p)))
(let ((p (open-output-bytevector)))
  (write-bytevector #u8(1 2 3) p)
  (test #u8(1 2 3) (get-output-bytevector p)))
(let ((p (open-output-bytevector)))
  (write-bytevector #u8(1 2 3 4 5) p 3)
  (test #u8(4 5) (get-output-bytevector p)))

;;; Bug 2: textual-port? on binary ports
(test #t (textual-port? (open-input-string "hello")))
...
(let ((p (open-binary-input-file "/tmp/kaappi-deferred-test.bin")))
  (test #f (textual-port? p))
  (test #t (binary-port? p))
  (close-port p))
...
(test #t (not (eq? (interaction-environment) #t)))
...
(test #t (= (string-ci-hash "hello") (string-ci-hash "HELLO")))
(test #t (= (string-ci-hash "abc") (string-ci-hash "ABC")))
;; Mixed case
(test #t (= (string-ci-hash "Hello") (string-ci-hash "hELLO")))

;;; Bug 8: load raises file-error
(test #t (guard (e (#t (file-error? e)))
