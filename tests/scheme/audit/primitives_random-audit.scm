...
(import (scheme base) (scheme read) (srfi 27) (scheme process-context) (srfi 64))

(test-begin "primitives_random audit")

;;; --- random-source? ---
(test-equal #t (random-source? (make-random-source)))
(test-equal #f (random-source? 42))
(test-equal #f (random-source? "rs"))
(test-equal #f (random-source? #f))
(test-equal #f (random-source? '()))
...
(test-equal #t (random-source? default-random-source))
...
(let* ((d default-random-source)
  ...
  (test-equal x1 (random-integer 1000000)))

;;; --- random-integer ---
;; n = 1 has only one possible result
(test-equal 0 (random-integer 1))
...
(let ((r (random-integer (expt 2 40))))
  (test-equal #t (and (>= r 0) (< r (expt 2 40)))))
...
(test-equal #t (let ((r (random-integer (expt 2 64))))
                 (and (>= r 0) (< r (expt 2 64)))))

;; bound whose top limb has MSB set (top_bits = 64)
(test-equal #t (let ((r (random-integer (expt 2 127))))
                 (and (>= r 0) (< r (expt 2 127)))))
(test-equal #t (let ((r (random-integer (- (expt 2 128) 1))))
                 (and (>= r 0) (< r (- (expt 2 128) 1)))))

;; invalid arguments raise catchable errors
(test-equal #t (guard (e (#t (error-object? e))) (random-integer 0) #f))
(test-equal #t (guard (e (#t (error-object? e))) (random-integer -5) #f))
(test-equal #t (guard (e (#t (error-object? e))) (random-integer "10") #f))
(test-equal #t (guard (e (#t (error-object? e))) (random-integer 'ten) #f))
...
(let* ((s1 (make-random-source))
  ...
  (test-equal (r1 1000000) (r2 1000000))
  (test-equal (r1 1000000) (r2 1000000))
  (test-equal (r1 1000000) (r2 1000000)))
...
(let* ((s (make-random-source))
  ...
  (test-equal 0 (rand 1))
  (let ((r (rand 100)))
    (test-equal #t (and (>= r 0) (< r 100))))
  (test-equal #t (guard (e (#t (error-object? e))) (rand 0) #f))
  (test-equal #t (guard (e (#t (error-object? e))) (rand -1) #f))
  (test-equal #t (guard (e (#t (error-object? e))) (rand 1.5) #f))
  ;; bignum range must be accepted (same requirement as random-integer)
  (test-equal #t (let ((r (rand (expt 2 64)))) (and (>= r 0) (< r (expt 2 64)))))
  )
...
(let* ((s (make-random-source))
  ...
  (let ((r (rand)))
    (test-equal #t (and (inexact? r) (> r 0) (< r 1)))))
...
(test-equal #t (exact? ((random-source-make-reals (make-random-source) 1/10))))
...
(let* ((s1 (make-random-source))
  ...
  (let ((r1 (random-source-make-integers s1))
        (r2 (random-source-make-integers s2)))
    (test-equal #f (and (= (r1 (expt 2 40)) (r2 (expt 2 40)))
                        (= (r1 (expt 2 40)) (r2 (expt 2 40)))
                        (= (r1 (expt 2 40)) (r2 (expt 2 40)))))))
(test-equal #t (guard (e (#t (error-object? e))) (random-source-randomize! 42) #f))
...
(test-equal #t (begin (random-source-pseudo-randomize!
...
(let* ((s (make-random-source))
  ...
  (let* ((st (random-source-state-ref s))
    ...
    (test-equal a (rand 1000000))
    (test-equal b (rand 1000000))))
