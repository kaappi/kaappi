;;; SRFI 264: String Syntax for Scheme Regular Expressions — conformance tests.
;;;
;;; Adapted from the SRFI 264 reference test suite (ssre-tests.scm) by
;;; Sergei Egorov. The 2754 test cases below (test-ssre = string->SRE,
;;; test-sre = SRE->string) exercise the parser and unparser directly and are
;;; kept verbatim from upstream; only the (import ...) prologue and the
;;; exit-on-failure epilogue are Kaappi-specific.
;;;
...
;; Kaappi: signal failure via output pattern + nonzero exit.
(when (< *tests-passed* *tests-run*)
  (display (- *tests-run* *tests-passed*))
  (display " failures\n")
  (exit 1))
