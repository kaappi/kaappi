...
(import (scheme base) (srfi 87) (scheme process-context) (srfi 64))

(test-begin "srfi-87")

;; => in a matching clause receives the key
(test-equal 20 (case 2 ((1 2 3) => (lambda (x) (* x 10))) (else 0)))
(test-equal 'two (case 2
                   ((1) => (lambda (x) 'one))
                   ((2) => (lambda (x) 'two))
                   (else 'other)))

;; => in the else clause receives the key
(test-equal 9 (case 9 ((1 2) => (lambda (x) 'small)) (else => (lambda (x) x))))
(test-equal 'composite (case 6
                         ((2 3 5 7) 'prime)
                         (else => (lambda (x) 'composite))))

;; mixing plain and => clauses
(test-equal 'plain (case 1 ((1) 'plain) ((2) => (lambda (x) 'arrow)) (else 'no)))
(test-equal 4 (case 2 ((1) 'plain) ((2) => (lambda (x) (* x x))) (else 'no)))
