...
;;; not against the implementation.  Covers every name exported by the six
;;; libraries: (srfi 166 base), (srfi 166), (srfi 166 pretty),
;;; (srfi 166 columnar), (srfi 166 unicode) and (srfi 166 color).
;;;
...
;;;
;;; `fn` and `with` are macros (see #2054): `fn` binds state variables into a
;;; lexical environment, and `with` dynamically binds them for its body while
;;; restoring only the variables it bound (so col/row survive).
...
(test-equal "numeric: the radix state variable supplies the default radix"
  "ff" (show #f (with (( radix 16)) (numeric 255))))
...
(test-equal "numeric: decimal-sep replaces the decimal point"
  "1,5" (show #f (with (( decimal-sep ",")) (numeric 1.5 10 1))))
...
(test-equal "numeric: the precision state variable reaches a bare number"
  "2" (show #f (with (( precision 0)) 1.5)))
(test-equal "numeric: the precision state variable reaches written"
  "0.143" (show #f (with (( precision 3)) (written 1/7))))
...
(test-equal "padded: the pad-char state variable is honoured"
  "..abc" (show #f (with (( pad-char #\.)) (padded 5 "abc"))))
...
(test-equal "trimmed/both: the ellipsis state variable is added on both sides"
  "_bcd_" (show #f (with (( ellipsis "_")) (trimmed/both 5 "abcdef"))))

(test-equal "trimmed: no truncation leaves the ellipsis unused"
  "abc" (show #f (with (( ellipsis "...")) (trimmed 5 "abc"))))
...
(test-equal "padded: the string-width state variable is used for measurement"
  "ab|"
  (show #f (with (( string-width (lambda (s) (* 2 (string-length s)))))
                 (padded/right 4 "ab"))
        "|"))
...
(test-assert "fn: the spec's own (fn (col) col) syntax works"
  (ok? (lambda () (show #f "column: " (fn (col) col)))))
(test-assert "with: the spec's own ((state-var value)) syntax works"
  (ok? (lambda () (show #f 1.5 " " (with ((precision 0)) 1.5)))))
...
(test-equal "with: a bound state variable is visible to the body"
  "..hi" (show #f (with (( pad-char #\.)) (padded 4 "hi"))))
...
(test-equal "with: output position survives the form (col not restored)"
  "abcd  x"
  (show #f (with (( pad-char #\.)) "abcd") (space-to 6) "x"))
(test-equal "with: fl after a with sees the real column"
  "a\nb\nZ"
  (show #f (with (( pad-char #\.)) "a" nl "b") fl "Z"))
...
(test-equal "output: binding it intercepts string output"
  "XXX"
  (show #f (with (( output (lambda (s) (output-default "XXX")))) "abc")))
...
(test-equal "wrapped/char: splits mid-word at the width"
  "abcde\nfghij"
  (show #f (with (( width 5)) (wrapped/char "abcdefghij"))))
...
(test-assert "inventory: make-state-variable is exported" (ok? (lambda () make-state-variable)))
(test-assert "inventory: the writer state variable is exported" (ok? (lambda () writer)))
(test-assert "inventory: substring/width is exported" (ok? (lambda () substring/width)))
(test-assert "inventory: substring/preserve is exported" (ok? (lambda () substring/preserve)))
(test-assert "inventory: decimal-align is exported" (ok? (lambda () decimal-align)))
(test-assert "inventory: word-separator? is exported" (ok? (lambda () word-separator?)))
(test-assert "inventory: ambiguous-is-wide? is exported" (ok? (lambda () ambiguous-is-wide?)))
(test-assert "inventory: pretty-with-color is exported" (ok? (lambda () pretty-with-color)))
(test-assert "inventory: string-terminal-width/wide is exported"
  (ok? (lambda () string-terminal-width/wide)))
(test-assert "inventory: substring-terminal-width/wide is exported"
  (ok? (lambda () substring-terminal-width/wide)))
(test-assert "inventory: substring-terminal-preserve is exported"
  (ok? (lambda () substring-terminal-preserve)))
