...
The documents below fall into distinct genres. **Guides** are evergreen and
must be kept current as the code changes. **Design decisions** and
**postmortems** are point-in-time records: each carries a status and date,
and only the status line is updated after the fact. **Reference notes**
document a subsystem's current shape. Open bugs belong in the issue
tracker, not here — a doc is only warranted when an investigation produced
...
| Document | Decision |
|----------|----------|
| [continuation-strategy.md](continuation-strategy.md) | Native codegen uses direct-style IR; `call/cc` side-exits to the bytecode VM |
| [self-tail-call-optimization.md](self-tail-call-optimization.md) | Dedicated `self_tail_call` opcode (Option A shipped; Option B attempted and reverted) |
...
| Document | Bug | Status |
|----------|-----|--------|
| [gc-reachability-bug.md](gc-reachability-bug.md) | String literals freed mid-execution — unrooted source datum during read/compile | Fixed 2026-06-17 |
| [global-cache-not-gc-traced.md](global-cache-not-gc-traced.md) | `Function.global_cache` not traced by `markValue` | Fixed 2026-06-17 |
| [fixnum-overflow-promotion.md](fixnum-overflow-promotion.md) | Arithmetic results in the fixnum/i64 gap silently wrapped | Fixed 2026-06-18 |
| [complex-number-test-precision.md](complex-number-test-precision.md) | `test-approx=?` didn't compare complex numbers component-wise | Fixed 2026-06-18 |
| [deep-recursion-register-overflow.md](deep-recursion-register-overflow.md) | Fixed-capacity register file overflowed under deep non-tail recursion | Fixed 2026-06-30 |
...
- **Recording a design decision?** New file, one decision per file. State
  the decision and its date in the first paragraph, then the analysis.
- **Writing up a fixed bug?** New postmortem with a `## Status` section
