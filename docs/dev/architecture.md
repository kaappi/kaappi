...
| **VM** | `vm.zig` + 10 sub-modules | Executes bytecode with a growable register file, call frame stack, exception handler stack, and dynamic-wind stack (all heap-allocated, double-on-overflow; exceeding a hard cap is an uncatchable KP3008). First-class continuations via stack copying, plus a stepping debugger. See [vm.md](vm.md). |
...
| **Primitives** | 32 `primitives_*.zig` files | 719 built-in procedures organized by domain. |
