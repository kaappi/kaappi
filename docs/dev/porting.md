...
| Subprocess backend | `src/process_posix.zig`, `src/process_win.zig` (selected in `src/primitives_process.zig`) | `(kaappi process)`'s syscalls: spawn, reap, signal, process group. `primitives_process.zig` holds everything platform-independent and picks one backend at comptime, so a new OS supplies a third file (or, like WASI, is left out of `all_specs` and gates `(library (kaappi process))` false). The seam is the three types in `types_process.zig`, plus `types_process.reapNonBlocking` — which `reactor.zig` and `gc_sweep.zig` also call, so a new backend must be reachable from there too. |
...
- [ ] Add a `release.yml` matrix row: `target`, `artifact`, `exe_ext`,
      ...
      A Linux row also needs `zig_target: <arch>-linux-gnu.2.28` — Zig's
      bare `<arch>-linux` default is musl-static, which cannot `dlopen`,
      so the shipped binary rejects `(kaappi ffi)` and every C-extension
      package with it (#1783; riscv64 repeated it in #2595 because its
      row was added without one) — and a leg in the `linux-ffi-smoke`
      job, which is the only thing that notices. Without a native
      runner, the leg runs the artifact under QEMU inside a pinned
      container (the riscv64 leg is the pattern).
