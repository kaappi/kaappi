...
(define (%test-display-header suite-name)
  (display (string #\% #\% #\% #\% #\space))
  (display (string #\S #\t #\a #\r #\t #\i #\n #\g #\space))
  (display (string #\t #\e #\s #\t #\space))
  (display suite-name)
  (newline))

(define (test-on-group-begin-simple runner suite-name count)
  (if (null? (test-runner-group-stack runner))
      (%test-display-header suite-name))
  #f)
...
(define (%test-final-report-simple runner port)
  (let ((pass  (test-runner-pass-count runner))
    ...
    (if (> pass 0)
        (begin (display pass port)
               (display (string #\space #\p #\a #\s #\s #\e #\d) port)
               (newline port)))
    (if (> fail 0)
        (begin (display fail port)
               (display (string #\space #\F #\A #\I #\L #\E #\D) port)
               (newline port)))
    (if (> xfail 0)
        (begin (display xfail port)
               (display (string #\space #\e #\x #\p #\e #\c #\t #\e #\d #\space
                                #\f #\a #\i #\l) port)
               (newline port)))
    (if (> xpass 0)
        (begin (display xpass port)
               (display (string #\space #\u #\n #\e #\x #\p #\e #\c #\t #\e #\d
                                #\space #\p #\a #\s #\s) port)
               (newline port)))
    (if (> skip 0)
        (begin (display skip port)
               (display (string #\space #\s #\k #\i #\p #\p #\e #\d) port)
               (newline port)))))
        ...
        (let* ((results (test-result-alist runner))
          ...
          (if (or source-file source-line)
              (begin
                ...
                (display (string #\: #\space))))
          (if (eq? kind 'xpass)
              (display (string #\X #\P #\A #\S #\S))
              (display (string #\F #\A #\I #\L)))
          (if test-name
              (begin
                (display (string #\space))
