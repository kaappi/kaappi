(define-library (srfi 152)
  ...
  (begin
    ...
    (define (string-contains-right s1 s2 . args)
      (let* ((start1 (if (pair? args) (car args) 0))
             (end1 (if (and (pair? args) (pair? (cdr args)))
                       (cadr args) (string-length s1)))
             (start2 (if (and (pair? args) (pair? (cdr args)) (pair? (cddr args)))
             ...
             (end2 (if (and (pair? args) (pair? (cdr args)) (pair? (cddr args))
                            (pair? (cdddr args)))
                       (cadddr args) (string-length s2)))
             (sub (substring s2 start2 end2))
             (sublen (string-length sub)))
        (let loop ((i (- end1 sublen)))
          (cond
            ((< i start1) #f)
            ((string=? (substring s1 i (+ i sublen)) sub) i)
            (else (loop (- i 1)))))))

    (define (string-take-while s pred . args)
      (let* ((start (if (pair? args) (car args) 0))
             (end (if (and (pair? args) (pair? (cdr args)))
                      (cadr args) (string-length s)))
             (idx (string-skip s pred start end)))
        (if idx (substring s start idx) (substring s start end))))

    (define (string-take-while-right s pred . args)
      (let* ((start (if (pair? args) (car args) 0))
             (end (if (and (pair? args) (pair? (cdr args)))
                      (cadr args) (string-length s)))
             (idx (string-skip-right s pred start end)))
        (if idx
            (substring s (+ idx 1) end)
            (substring s start end))))

    (define (string-drop-while s pred . args)
      (let* ((start (if (pair? args) (car args) 0))
             (end (if (and (pair? args) (pair? (cdr args)))
                      (cadr args) (string-length s)))
             (idx (string-skip s pred start end)))
        (if idx (substring s idx end) "")))

    (define (string-drop-while-right s pred . args)
      (let* ((start (if (pair? args) (car args) 0))
             (end (if (and (pair? args) (pair? (cdr args)))
                      (cadr args) (string-length s)))
             (idx (string-skip-right s pred start end)))
        (if idx (substring s start (+ idx 1)) "")))

    (define (string-break s pred . args)
      (let* ((start (if (pair? args) (car args) 0))
             (end (if (and (pair? args) (pair? (cdr args)))
                      (cadr args) (string-length s)))
             (idx (string-index s pred start end)))
        (if idx
            (values (substring s start idx)
                    (substring s idx end))
            (values (substring s start end) ""))))

    (define (string-span s pred . args)
      (let* ((start (if (pair? args) (car args) 0))
             (end (if (and (pair? args) (pair? (cdr args)))
                      (cadr args) (string-length s)))
             (idx (string-skip s pred start end)))
