...
;;; hands the bounded, always-proper range produced by (srfi 274
;;; internal)'s range-list to the underlying one-argument converter — same
;;; result, one intermediate list, and it keeps the library from importing
;;; all twelve full (srfi 160 <type>) surfaces just for their unfold.
;;;
  ...
  (begin
    (define (converter who basic)
      (case-lambda
        ...
        ((lst start end)
         (basic (range-list who lst start end)))))
