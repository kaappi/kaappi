...
Of the 208 final SRFIs in the registry, 150 are implemented, 38 are tracked for
future implementation (issues #1694–#1705 minus what Phases 1–3 closed, across
Phase 4 — SRFI 181 landed in Phase 3 as custom ports, #1727, with transcoded
ports split into their own follow-up, #1729), and 20 are excluded — see
`docs/dev/srfi-exclusions.md` for the full rationale (7 meta/ecosystem SRFIs
already covered by existing features, 10 non-standard reader syntax SRFIs that
would fundamentally alter the parser or need typed-array infrastructure that
doesn't exist, 2 SRFIs — 206 and 212 — whose own spec text states a portable
syntax-rules-only implementation isn't possible, and 1 SRFI — 208 — whose own
spec text states the same about raw NaN bit-pattern access, which Kaappi's
