...
Of the 208 final SRFIs in the registry, 177 are implemented, 1 is tracked
for future implementation (150 — issue #1810: its stated blockers, SRFI
147+148, both shipped, so its exclusion entry was retired when issue
#1699 closed), and 30 are excluded —
see `docs/dev/srfi-exclusions.md` for the full rationale. Issue #1699
...
facility actually wanted is exactly `(srfi 211 explicit-renaming)`. Issue #1694 (the
...
non-obvious integration points: (1) `vm_library.copyTransformerFreeRefs`
