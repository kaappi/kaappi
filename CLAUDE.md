...
Of the 208 final SRFIs in the registry, 170 are implemented, 9 are tracked for
future implementation (issue #1694's remaining array-family SRFI — 231
(supersedes 179) — a third, much larger, unrelated redesign from SRFI
25/164/63 (118 bindings: 101 procedures/parameters plus 17 storage-class
singletons) with a three-tier array/mutable-array/specialized-array model
and no textual relationship to any of them at all; SRFI 58's reader/writer
array-literal syntax is separately excluded already, see
`docs/dev/srfi-exclusions.md`, though its stated blocker — no typed array
infrastructure to build on — no longer fully holds now that SRFI 4/160/25/
164/63 all exist, worth re-examining if 58 is ever picked up);
...
independent-heap-per-thread SRFI-18 doesn't have, and 1 SRFI — 47 —
superseded outright by the now-implemented SRFI 63, whose own page states
the supersession directly and whose procedure set is a strict superset of
47's with identical signatures throughout).
