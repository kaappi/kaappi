...
Of the 208 final SRFIs in the registry, 169 are implemented, 11 are tracked for
future implementation (issue #1694's remaining array-family SRFIs — 47, 58,
63, 179, 231 — split into two lineages plus a reader-syntax SRFI, none
related to the vector family or to SRFI 25/164 (both now implemented, see
below): SRFI 47/63 (63 supersedes 47) is a lineage incompatible with 25/164,
confirmed by both SRFIs' own Issues sections (`array-set!`'s new-value
argument is last in 25/164 vs. immediately after `array` in 47/63 — a
silent-data-corruption-class conflict, not just argument-order noise — and
`make-array`'s first argument is a bounds-shape object in 25/164 vs. a
type/fill-value prototype in 47/63); SRFI 231 (supersedes 179) is a third,
much larger, unrelated redesign (118 bindings: 101 procedures/parameters
plus 17 storage-class singletons) with a three-tier
array/mutable-array/specialized-array model and no textual relationship to
25/164 at all; SRFI 58's reader/writer array-literal syntax is written
specifically against 47/63's own type-tag naming and isn't reusable as-is
against 25/164/231 without redesigning the tag mapping);
...
#1703 and #1702 closed in full in Phase 4), and 28 are excluded — see
