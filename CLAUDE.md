...
Of the 208 final SRFIs in the registry, 168 are implemented, 12 are tracked for
future implementation (issue #1694's remaining array-family SRFIs — 47, 58, 63,
164, 179, 231 — split into two lineages plus a reader-syntax SRFI, none
related to the vector family: SRFI 164 is a documented, compatible
*extension* of the now-implemented SRFI 25 (identical shape representation,
`share-array` copied verbatim) and is a comparatively cheap follow-on (23
bindings, the same 10 as 25 plus 13 new); SRFI 47/63 (63 supersedes 47) is a
separate, incompatible lineage confirmed by both SRFIs' own Issues sections
(`array-set!`'s new-value argument is last in 25/164 vs. immediately after
`array` in 47/63 — a silent-data-corruption-class conflict, not just
argument-order noise — and `make-array`'s first argument is a bounds-shape
object in 25/164 vs. a type/fill-value prototype in 47/63); SRFI 231
(supersedes 179) is a third, much larger, unrelated redesign (118 bindings:
101 procedures/parameters plus 17 storage-class singletons) with a
three-tier array/mutable-array/specialized-array model and no textual
