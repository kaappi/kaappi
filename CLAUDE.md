...
Of the 208 final SRFIs in the registry, 175 are implemented, 3 are tracked
for future implementation (72, 211, 213 — issue #1699, "Implement
SRFI macro & syntax extension libraries": all 3 need expander or compiler
changes, e.g. 72's explicit-renaming macros; 139 and 149,
originally grouped here for their syntax-parameter and
syntax-rules-template-extension mechanisms respectively, needed no engine
changes at all and shipped directly, 147 (custom macro
transformers) DID need one, and 148 (eager syntax-rules), the group's
hardest case, shipped last as pure portable Scheme over 147's mechanism
once six separate engine bugs it surfaced were fixed — see below for all
four), and 30 are
excluded —
...
(`compileLetSyntax`, `compiler_macro.zig`) already implements exactly
...
the innermost position — are both already correct in `expander.zig`'s
existing `instantiateEllipsis`. The spec's own prose gives no worked
...
(`compiler_macro.zig`) now route every transformer-spec through a new
