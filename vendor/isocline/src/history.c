...
// KAAPPI PATCH 2: see vendor/isocline/PATCHES.md
// Upstream clamps every history to 200 entries. Kaappi's `repl.history-length`
// defaults to 1000 and is user-configurable (src/config.zig), so the cap has to
// be a sanity bound rather than the policy. An explicit request is honored;
// a negative or absent one still lands on the upstream default.
#define IC_DEFAULT_HISTORY (200)
#define IC_MAX_HISTORY (1000000)
...
ic_private void history_load_from(history_t* h, const char* fname, long max_entries ) {
  ...
  // KAAPPI PATCH 2: honor an explicit request; clamp only absurd values.
  if (max_entries < 0) max_entries = IC_DEFAULT_HISTORY;
  if (max_entries > IC_MAX_HISTORY) max_entries = IC_MAX_HISTORY;
