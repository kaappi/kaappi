...
/// \defgroup complete_input Input completeness
/// Decide at Enter whether the input is a finished expression.
/// \{

/// Called when the user presses Enter. Return `true` if `input` is a complete
/// expression and should be submitted, or `false` to insert a newline and keep
/// editing — the way a Lisp prompt continues while parentheses are still open.
///
/// Upstream isocline has no such hook: Enter always submits unless the input
/// ends in the `multiline_eol` continuation character, and a newline is
/// otherwise entered explicitly with shift-tab / ctrl-J. Kaappi needs the
/// language, not the keyboard, to decide.
///
/// Not consulted when single-line mode is set (`ic_enable_multiline(false)`).
typedef bool (ic_is_complete_fun_t)(const char* input, void* arg);

/// Set the input-completeness callback. There can only be one; setting it
/// again replaces the previous one. Pass NULL to restore stock behavior.
