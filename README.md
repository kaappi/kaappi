...
137 SRFIs, a C FFI, OS threads and fibers, an LLVM native-code backend, a package
...
- **137 SRFIs** — 11 built-in, 124 as portable `.sld` libraries, plus SRFI 261 portable library references (`(srfi srfi-1)`, `(srfi lists-1)`) resolved in the importer and SRFI 226 as sub-libraries only (full list in [CONFORMANCE.md](CONFORMANCE.md))
...
137 SRFIs are supported. Some built-in SRFIs have minor coverage gaps (e.g.,
