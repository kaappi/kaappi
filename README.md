...
78 SRFIs, a C FFI, OS threads and fibers, an LLVM native-code backend, a package
...
- **78 SRFIs** — 9 built-in, 68 as portable `.sld` libraries, plus SRFI 261 portable library references (`(srfi srfi-1)`, `(srfi lists-1)`) resolved in the importer (full list in [CONFORMANCE.md](CONFORMANCE.md))
...
78 SRFIs are supported. Some built-in SRFIs have minor coverage gaps (e.g.,
