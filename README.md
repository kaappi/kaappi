...
82 SRFIs, a C FFI, OS threads and fibers, an LLVM native-code backend, a package
...
- **82 SRFIs** — 10 built-in, 71 as portable `.sld` libraries, plus SRFI 261 portable library references (`(srfi srfi-1)`, `(srfi lists-1)`) resolved in the importer (full list in [CONFORMANCE.md](CONFORMANCE.md))
...
82 SRFIs are supported. Some built-in SRFIs have minor coverage gaps (e.g.,
